...
<div id="content">
      ...
      <p class="nav_links"><b>{{= anchor('manager/review_feature/add/'.$review->id,lang('manager_review_features_add_feature')) }}</b></p>
      <p class="nav_links"><b>{{= anchor('manager/review/edit/'.$review->id,lang('manager_review_features_back_to_review')) }}</b></p>
      <p class="break">&nbsp;</p>
      ...
      <form id="form" class="myform" name="form" method="post" action="{{= base_url() . 'manager/review_features/edit/' . $review->id }}">
           {{ foreach ($allreviewfeatures as $index=>$result): }}
                  <p>
                        ...
                        <span>
                              <input class="input_right" type="text" name="value{{= $index }}" id="value{{= $index }}" value="{{= set_value('value', $result->value) }}"/>
...
<input type="submit" name="review_features_submit" style="width:300px" id="button" value="{{= lang('manager_review_features_form_submit_button') }}" />
      </form>
</div>